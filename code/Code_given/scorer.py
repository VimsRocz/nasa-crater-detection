...
def main():
    ...
    parser.add_argument(
        "--truth", type=str, default="./gt-files/test-gt.csv", help="path to gt file"
    )
    parser.add_argument(
        "--pred", type=str, default="./sample-data/solution.csv", help="path to prediction file"
    )
    ...
    truth = (
        truth_df.set_index("inputImage")
        .groupby(level="inputImage")
        .apply(lambda g: g.to_dict(orient="records"))
        .to_dict()
    )
    detections = (
        detections_df.set_index("inputImage")
        .groupby(level="inputImage")
        .apply(lambda g: g.to_dict(orient="records"))
        .to_dict()
    )
