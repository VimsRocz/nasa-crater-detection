...
class DatasetLoader:
    ...
    def validate_dataset(self, dataset_name: str) -> Tuple[bool, str]:
        ...
        return (
            True,
            f"Dataset '{dataset_name}' is valid ({dataset_info['altitude_count']} altitude folders found)",
        )
...
class CraterDetector:
    ...
    def __init__(
        self,
        min_semi_minor_axis=DEFAULT_MIN_SEMI_MINOR_AXIS,
    ...
    def filter_crater(self, ellipse_params: Optional[Tuple], image_shape: Tuple) -> bool:
        """
    ...
    def validate_crater_appearance(self, image: np.ndarray, ellipse_params: Tuple) -> float:
        """Validate if a detected region looks like a crater."""
        center_x, center_y, semi_major, semi_minor, rotation = ellipse_params

        if np.isnan(center_x) or np.isnan(center_y) or np.isnan(semi_major) or np.isnan(semi_minor):
            return 0.0
        ...
        cv2.ellipse(
            mask, local_center, (int(semi_major), int(semi_minor)), rotation, 0, 360, 255, -1
        )
    ...
    def classify_crater(
        self, ellipse_params: Tuple, image: np.ndarray, confidence: float = None
    ) -> int:
    ...
    def detect_craters_in_image(self, image_path: str, verbose: bool = False) -> List[Dict]:
    ...
    def process_dataset(self, data_folder: str, output_file: str, verbose: bool = True) -> bool:
        ...
        if dataset_key in loader.available_datasets:
            ...
            if info["exists"] and info["is_valid"]:
                ...
                if info.get("has_ground_truth"):
                    print("     Ground Truth: Available (use --evaluate flag)")
            elif info["exists"] and not info["is_valid"]:
                print("     Status: ⚠ Needs extraction (folder exists but no altitude/* structure)")
            else:
                print("     Status: Not found (may need to extract TAR archive)")
...
def compare_with_ground_truth(detections_file: str, ground_truth: pd.DataFrame) -> Dict:
...
def select_output_file(data_folder: str = None, default_name: str = "solution.csv") -> str:
    ...
    while True:
        try:
            choice = input(
                f"\nSelect option (0-{len(suggested_outputs)}) or enter custom path: "
            ).strip()
    ...
    try:
        ...
        for i in range(3):
            ...
            for j in range(num_craters):
                ...
                cv2.ellipse(
                    image, (center_x, center_y), (semi_major, semi_minor), rotation, 0, 360, 50, 3
                )
...
def analyze_detections(file_path):
    ...
    print("--- Potential Outliers (Ellipses larger than challenge filter) ---\\n")
    print(
        f"The detection script should already filter craters where (semi_major + semi_minor) >= {max_size_threshold:.1f} pixels."
    )
    ...
    if not large_craters.empty:
        ...
        print(
            large_craters[
                ["inputImage", "ellipseSemimajor(px)", "ellipseSemiminor(px)", "size_sum"]
            ].to_string()
        )
    else:
    ...
    if not elongated_craters.empty:
        ...
        print(
            elongated_craters[
                ["inputImage", "ellipseSemimajor(px)", "ellipseSemiminor(px)", "aspect_ratio"]
            ].to_string()
        )
    else:
...
Expected data structure within the specified data_folder:
  ...
  └── altitude02/
    ...
    parser.add_argument(
        "--verbose", action="store_true", default=True, help="Print detailed progress information"
    )
    parser.add_argument(
        "--canny1", type=int, default=50, help="First threshold for the Canny edge detector."
    )
    parser.add_argument(
        "--canny2", type=int, default=150, help="Second threshold for the Canny edge detector."
    )
    parser.add_argument(
        "--circularity",
        type=float,
        default=0.6,
        help="Circularity threshold for crater validation.",
    )
    ...
    detector = CraterDetector(
        canny_th1=args.canny1, canny_th2=args.canny2, circularity_threshold=args.circularity
    )
    ...
    print("=" * 70)
    success = detector.process_dataset(args.data_folder, args.output, verbose=args.verbose)
    ...
    if args.evaluate:
        ...
        if ground_truth_df is not None:
            print("✓ Loaded ground truth from truth folders")
            ...
            try:
                ...
                truth_for_scoring = (
                    ground_truth_df.set_index("inputImage")
                    .groupby(level="inputImage")
                ...
                detections_for_scoring = (
                    detections_for_scoring_df.set_index("inputImage")
                    .groupby(level="inputImage")
                    .apply(lambda g: g.to_dict(orient="records"))
                    .to_dict()
                )
